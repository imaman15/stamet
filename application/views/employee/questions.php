...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4 animated zoomIn fast">
        <div class="card-header py-3">
            <button type="button" class="btn btn-primary" onclick="add_ratque()">
                Tambah Pertanyaan
            </button>
        </div>
        <div class="card-body">
...
<!-- Modal Add/Update -->
<div class="modal fade" id="ratque" tabindex="-1" role="dialog" aria-labelledby="ratqueLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title font-weight-bolder" id="ratqueLabel">Data Pertanyaan</h4>
            ...
            <div class="modal-body form">
                <form action="#" id="form">
                    <input type="hidden" name="ratque_id" />
                    <div class="form-group row">
                        <label for="description" class="col-sm-3 col-form-label text-sm-right font-weight-bold">Pertanyaan</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="description" id="description" placeholder="Pertanyaan" value="">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dataTable').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('employee/questions/list') ?>",
                "type": "POST"
            },
    ...
    function add_ratque() {
        ...
        $('#ratque').modal('show'); // show bootstrap modal
        $('#ratqueLabel').text('Tambah Data'); // Set Title to Bootstrap modal title
    }

    // Edit Data
    function edit_ratque(id) {
        save_method = 'update';
        $('#form')[0].reset(); // reset form on modals
        $('.form-control').removeClass('is-invalid'); // clear error class
        $('.invalid-feedback').empty(); // clear error string
        ...
        $.ajax({
            url: "<?php echo site_url('employee/questions/view') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {

                $('[name="ratque_id"]').val(data.ratque_id);
                $('[name="description"]').val(data.description);
                $('[name="information"]').val(data.information);
                $('#ratque').modal('show'); // show bootstrap modal when complete loaded
    ...
    function save() {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('employee/questions/add') ?>";
            act_success = "ditambahkan";
            act_danger = "menambah";
        } else {
            url = "<?php echo site_url('employee/questions/update') ?>";
            act_success = "diedit";
            act_danger = "mengedit";
        }
    ...
    function delete_ratque(id) {
        $('#deleteData').modal('show'); // show bootstrap modal
        $('#btn-delete').hide();
        $.ajax({
            url: "<?php echo site_url('employee/questions/viewdelete') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
    ...
    function deleteBtn(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('employee/questions/delete') ?>/" + id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
